...
def test_zip_folder_async_task_success(client: TestClient, case_dir: Path) -> None:
...
def test_zip_folder_async_task_failed_missing_input(client: TestClient, case_dir: Path) -> None:
    payload = {"input_dir": str(case_dir / "missing_input")}

    submit_resp = client.post("/api/v1/preprocess/zip-folder/async", json=payload)
    assert submit_resp.status_code == 202
...
def test_zip_folder_async_uses_public_base_url(
    case_dir: Path,
    monkeypatch,
    isolated_runtime,
) -> None:
...
def test_zip_folder_async_callback_success(
    client: TestClient,
    case_dir: Path,
    monkeypatch,
) -> None:
...
def test_zip_folder_async_callback_failed_delivery(
    client: TestClient,
    case_dir: Path,
    monkeypatch,
) -> None:
...
def test_zip_folder_async_callback_fallback_to_get(
    client: TestClient,
    case_dir: Path,
    monkeypatch,
) -> None:
