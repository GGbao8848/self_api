...
def _build_system_prompt(tools: list[ToolSpec]) -> str:
    ...
    return (
        ...
        "15. For aggregate-nested-dataset, include output_dir as <input_dir>_dataset unless user gives one.\n"
        "16. For zip-folder, if output_zip_path is omitted, use <input_dir>.zip.\n"
        "17. For unzip-archive, use archive_path and include output_dir only when the user gives one or a custom extraction location is needed.\n"
        "18. For move-path and copy-path, use source_path and target_dir exactly; do not rewrite them as input_dir.\n"
        "19. For reset-yolo-label-index, use input_dir pointing to labels/, a dataset root, or a parent directory containing nested labels directories.\n"
        "20. For voc-bar-crop, include output_dir as <input_dir>_voc-bar-crop unless user gives one.\n"
        "21. For restore-voc-crops-batch, required keys are original_images_dir, original_xmls_dir, edited_crops_images_dir, edited_crops_xmls_dir, and optionally output_dir.\n"
        "Available tools:\n"
        f"{tools_text}\n"
        "Argument hints:\n"
        f"{argument_text}"
