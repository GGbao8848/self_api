...
@router.post("/publish-yolo-dataset", response_model=PublishYoloDatasetResponse)
def publish_yolo_dataset(payload: PublishYoloDatasetRequest) -> PublishYoloDatasetResponse:
    try:
...
@router.post(
    "/publish-yolo-dataset/async",
    response_model=AsyncTaskSubmitResponse,
    status_code=202,
)
def publish_yolo_dataset_async(
    payload: PublishYoloDatasetAsyncRequest,
    request: Request,
) -> AsyncTaskSubmitResponse:
    task_type = "publish_yolo_dataset"
    callback_url = str(payload.callback_url) if payload.callback_url else None
    sync_payload = PublishYoloDatasetRequest(
        **payload.model_dump(exclude={"callback_url", "callback_timeout_seconds"})
    )
    task_id = submit_task(
        task_type=task_type,
...
def yolo_train(payload: YoloTrainRequest) -> YoloTrainResponse:
    try:
        return run_yolo_train(payload)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
